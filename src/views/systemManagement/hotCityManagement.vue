        ...
        <Modal v-model="addModal"
            ...
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
                <FormItem label="省份" prop="p_name" :label-width="85">
                    <Select v-model="formValidate.p_name" @on-change="PSelChange" placeholder="请选择">
                        <Option v-for="item in PList" :value="item.code" :key="item.id">{{ item.name }}</Option>
                    </Select>
                </FormItem>
...
export default {
  ...
  data() {
    ...
    var pValid = (rule, value, callback) => {
      if (!value) {
        return callback(new Error("请填写完整"));
      } else {
        callback();
    ...
    return {
      ...
      ruleValidate: {
        // 定义表单的校验规则
        p_name: [
          { validator: pValid, trigger: "change" }
        ],
        ...
        hcm_sort: [{ required: true, message: "请输入排序", trigger: "blur" }]
      },
  ...
  methods: {
    ...
    async getPList() {
      let params = {};
      let { data } = await hotCityPList(params);
      console.log(data);
      this.PList = data;
    },
    async getCList(p_code) {
      ...
      let { data } = await hotCityCList(params);
      console.log(data);
      this.CList = data;
    },
    //省下拉框:被选择时触发
